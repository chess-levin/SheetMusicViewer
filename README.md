...
Ein einfacher Windows PDF-Betrachter für Notenblätter. Es werden immer zwei Blätter hochkant nebeneinander dargestellt. 
Beim Blättern rutscht eineSeite raus, so behält man beim Spielen immer die Fortsetzunbg im Blick.
...
Unter den Releases ist die aktuelle Version zu finden. 
Das zip-File enthält eine vollständig eigenständige Version mit dem .NET Framework. 
Ist das Framework schon vorhanden kann die andere Version ausgeführt werden. 
Sollte .NET fehlen oder nicht passen, wird es beim Starten nachinstalliert.

## Projekt auschecken und bauen
...
Der Workflow [Publish Windows release](.github/workflows/release.yml) baut beide Varianten auf einem Windows-Runner und veröffentlicht sie gemeinsam als `release.zip` im GitHub Release. Die ZIP-Datei enthält die Ordner `standalone` und `framework-dependent` einschließlich der jeweiligen SBOM- und Lizenzdateien sowie `README.md` und `LICENSE` im ZIP-Hauptverzeichnis.
